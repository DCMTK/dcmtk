...
#endif
{
  ...
  if (userKey == NULL)
    DCMPSTAT_ERROR("Cannot verify user password: unknown user or undefined private key file");
  else
  {
    ...
    if (in)
    {
      if (BIO_read_filename(in, filename.c_str()) > 0)
      {
        ...
        DCMPSTAT_ERROR("Cannot verify user password: private key file not found");
      BIO_free(in);
    }
