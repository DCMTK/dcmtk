...
I2DJpegUtil::I2DJpegUtil(bool useJPEGLS)
{
    m_useJPEGLS = useJPEGLS;
}

OFCondition I2DJpegUtil::openFile(const OFString& filename)
{
...
void I2DJpegUtil::setExtSeqSupport(const OFBool enabled)
{
    if(!m_useJPEGLS)
    m_disableExtSeqTs = !enabled;
}

void I2DJpegUtil::setProgrSupport(const OFBool enabled)
{
    if(!m_useJPEGLS)
...
void I2DJpegUtil::setInsistOnJFIF(const OFBool enabled)
{
    if(!m_useJPEGLS)
    m_insistOnJFIF = enabled;
}

void I2DJpegUtil::setKeepAPPn(const OFBool enabled)
{
    m_keepAPPn = enabled;
}

void I2DJpegUtil::setKeepCOM(const OFBool enabled)
{
    m_keepCOM = enabled;
}

OFCondition I2DJpegUtil::readPixelData(OFString const & imageFile, Uint16& rows,
                                         Uint16& cols,
                                         ...
                                         E_TransferSyntax& ts)
{
    ...
    OFCondition cond = openFile(imageFile);
    ...
    while (entry != m_jpegFileMap.end())
    {
        if (isSOFMarker((*entry)->marker, m_useJPEGLS))
        {
    ...
    ts = associatedTS(jpegEncoding, m_useJPEGLS, nearLossless);
...
OFCondition I2DJpegUtil::getSOFImageParameters(const JPEGFileMapEntry& entry,
                                                 Uint16& imageWidth,
                                                 Uint16& imageHeight,
                                                 Uint16& samplesPerPixel,
...
OFCondition I2DJpegUtil::getSOSImageParameters(const JPEGFileMapEntry& entry,
                                                 Uint8& nearLossless)
{
...
OFCondition I2DJpegUtil::getJFIFImageParameters(
...
OFCondition I2DJpegUtil::extractRawJPEGStream(char*& pixelData, Uint32& pixLength)
{
...
OFCondition I2DJpegUtil::createJPEGFileMap()
{
    ...
    while (cond.good())
    {
        ...
        if (cond.good())
        {
            ...
            entry->marker  = marker;
            m_jpegFileMap.push_back(entry);
            if (marker == E_JPGMARKER_SOS && !m_useJPEGLS)
            {
                // FIXME: reset this to OFFalse after the next marker?
                lastWasSOSMarker = OFTrue;
            }
            else if (marker == E_JPGMARKER_JPGN7 && m_useJPEGLS)
            {
                // FIXME: reset this to OFFalse after the next marker?
                lastWasSOSMarker = OFTrue;
            }
...
OFBool I2DJpegUtil::isRSTMarker(const E_JPGMARKER& marker)
{
    return ((marker >= E_JPGMARKER_RST0) && (marker <= E_JPGMARKER_RST7));
}

OFBool I2DJpegUtil::isSOFMarker(const E_JPGMARKER& marker, OFBool const useJPEGLS)
{
    if(useJPEGLS)
...
OFString I2DJpegUtil::jpegMarkerToString(const E_JPGMARKER& marker, OFBool const useJPEGLS)
{
    switch (marker)
    {
        ...
        default:
            ...
            {
                if(marker == E_JPGMARKER_JPGN7 && useJPEGLS)
...
int I2DJpegUtil::read1Byte(Uint8& result)
{
    int c;
    c = jpegFile.fgetc();
    if (c == EOF)
...
int I2DJpegUtil::read2Bytes(Uint16& result)
{
    int c1, c2;
    c1 = jpegFile.fgetc();
    if (c1 == EOF)
...
OFCondition I2DJpegUtil::nextMarker(const OFBool& lastWasSOSMarker, E_JPGMARKER& result)
{
    Uint8 c;
...
OFCondition I2DJpegUtil::firstMarker(E_JPGMARKER& result)
{
    Uint8 c1, c2;
...
OFCondition I2DJpegUtil::skipVariable()
...
OFCondition I2DJpegUtil::isJPEGEncodingSupported(const E_JPGMARKER& jpegEncoding) const
{
    OFCondition result;
    DCMDATA_LIBI2D_DEBUG("I2DJpegSource: Checking whether JPEG encoding is supported");
    DCMDATA_LIBI2D_DEBUG("I2DJpegSource:   Encoding: " << jpegMarkerToString(jpegEncoding, m_useJPEGLS));
    if(m_useJPEGLS)
    switch (jpegEncoding)
    {
        ...
        default:
            ...
            errMsg += jpegMarkerToString(jpegEncoding, m_useJPEGLS);
...
E_TransferSyntax I2DJpegUtil::associatedTS(const E_JPGMARKER& jpegEncoding, OFBool const useJPEGLS, Uint8 const nearLossless)
{
    if(useJPEGLS)
    switch (jpegEncoding)
    {
...
void I2DJpegUtil::debugDumpJPEGFileMap() const
{
    ...
    while (it != m_jpegFileMap.end())
    {
        DCMDATA_LIBI2D_DEBUG("I2DJpegSource:   Byte Position: 0x"
                             ...
                             << " | Marker: " << jpegMarkerToString((*it)->marker, m_useJPEGLS));
        it++;
    }
}

void I2DJpegUtil::clearMap()
{
...
void I2DJpegUtil::closeFile()
{
    jpegFile.fclose();
}

// close file and free dynamically allocated memory
I2DJpegUtil::~I2DJpegUtil()
{
