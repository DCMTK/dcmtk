...
OFBool DSRSCoordTreeNode::isEqual(const DSRDocumentTreeNode &node) const
{
    ...
    if (result)
    {
        ...
        result = DSRSpatialCoordinatesValue::isEqual(OFstatic_cast(const DSRSCoordTreeNode &, node).getValue());
    }
    return result;
}


OFBool DSRSCoordTreeNode::isNotEqual(const DSRDocumentTreeNode &node) const
{
    /* call comparison operator of base class (includes check of value type) */
    ...
    {
        /* it's safe to cast the type since the value type has already been checked */
        result = DSRSpatialCoordinatesValue::isNotEqual(OFstatic_cast(const DSRSCoordTreeNode &, node).getValue());
    }
    return result;
}
...
OFBool operator==(const DSRSCoordTreeNode &lhs,
