...
OFBool DSRTimeTreeNode::isEqual(const DSRDocumentTreeNode &node) const
{
    ...
    if (result)
    {
        ...
        result = DSRStringValue::isEqual(OFstatic_cast(const DSRTimeTreeNode &, node).getValue());
    }
    return result;
}


OFBool DSRTimeTreeNode::isNotEqual(const DSRDocumentTreeNode &node) const
{
    /* call comparison operator of base class (includes check of value type) */
    ...
    {
        /* it's safe to cast the type since the value type has already been checked */
        result = DSRStringValue::isNotEqual(OFstatic_cast(const DSRTimeTreeNode &, node).getValue());
    }
    return result;
}
    ...
    if (cursor.valid())
    {
        ...
        if (!doc.getStringFromNodeContent(cursor, tmpString).empty())
        {
            ...
            if (!tmpTime.setISOFormattedTime(tmpString) ||
                DcmTime::getDicomTimeFromOFTime(tmpTime, timeValue, tmpTime.hasSecond() /*seconds*/, OFTrue /*fraction*/).bad())
            {
                /* just report a warning, as we cannot return an error status */
                DCMSR_WARN("Cannot convert ISO formatted time value \"" << tmpString << "\" to DICOM format");
            }
        }
    }
    return timeValue;
...
OFBool operator==(const DSRTimeTreeNode &lhs,
                  const DSRTimeTreeNode &rhs)
{
