 ...
 *  Update Date:      $Date: 2011-04-28 17:51:20 $
...
public:
  ...
  /// Number of results to C-FIND request that were received
  Uint32 numResults() const;
...
public:
   ...
   */
  virtual OFCondition sendMOVERequest(T_ASC_PresentationContextID presID,
   ...
   *                             was received on.
   *  @param reponse       [in]  The C-MOVE response received.
   ...
   *  @param presContextID [in]  The presentation context ID where the response
   *                             was received on.
   *  @param reponse       [in]  The C-FIND response received.
   *  @param waitForNextResponse [out] Denotes whether SCU should try to
   *                                   receive another response. If set to
   ...
   *  @param presContextID [in]  The presentation context ID where the C-FIND-CANCEL
   *                             should be sent on.
   *  @return The current implementation always returns EC_Normal.
   */
  virtual OFCondition sendFINDCANCELRequest(Uint16 presContextID);

  /** NOTE: NOT IMPLEMENTED YET!
   *  Send C-MOVE-CANCEL and, therefore, ends the C-MOVE session, i.e.\ no further responses
   *  will be handled. A call to this function only makes sense if an association is open,
   *  the given presentation context represents a valid C-MOVE-enabled SOP class and usually
   *  only, if the last command send on that presentation context was a C-MOVE message.
   *  @param presContextID [in]  The presentation context ID where the C-MOVE-CANCEL
   *                             should be sent on.
   *  @return The current implementation always returns EC_Normal.
   */
  virtual OFCondition sendMOVECANCELRequest(Uint16 presContextID);
