 ...
 *  Update Date:      $Date: 2011-08-25 09:31:33 $
...
public:
  ...
  */
   ...
   *  @param request         [in] The incoming C-STORE request commandset
   ...
   *  function is not called if the SCU is in storage mode
   *  DCMSCU_STORAGE_IGNORE.
   *  @param filename The filename written
   *  @param sopClassUID The SOP Class UID of the object written
   *  @param sopInstanceUID The SOP Instance UID of the object written
   */
   ...
   *  handle to store objects that come in with the corresponding
   *  C-STORE rqeuests
   *  @param storeDir The directory to store to. It is checked
   *         in handleSTORERequest() whether the directory is
   *         writeable and readable. Per default, the received objects
...
protected:
   ...
   *  @param pcid    [in] The presentation context ID to be used.
   ...
   */
  virtual OFCondition sendCStoreResponse(T_ASC_PresentationContextID pcid,
                                         Uint16 status,
   ...
   *  @param pcid    [in] The presentation context to be used
   *  @param request [in] The corresponding C-STORE request
   *  @return EC_Normal if ignoring worked, error code otherwise.
   */
  virtual OFCondition ignoreSTORERequest(T_ASC_PresentationContextID* pcid,
                                         const T_DIMSE_C_StoreRQ& request);
