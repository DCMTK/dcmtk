...
extern "C"
{
  struct jpeg8_compress_struct;
  typedef struct jpeg8_compress_struct * j8_compress_ptr;
}
...
public:
   ...
   */
  void initDestination(jpeg8_compress_struct *cinfo);

  /** callback for IJG compress destination manager.
   *  Internal use only, not to be called by client code.
   *  @param cinfo pointer to compress info
   */
  int emptyOutputBuffer(jpeg8_compress_struct *cinfo);

  /** callback for IJG compress destination manager.
   *  Internal use only, not to be called by client code.
   *  @param cinfo pointer to compress info
   */
  void termDestination(jpeg8_compress_struct *cinfo);
