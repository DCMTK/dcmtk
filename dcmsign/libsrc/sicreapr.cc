...
OFBool SiCreatorProfile::isAllowableMACType(E_MACType macType) const
{
...
OFBool SiCreatorProfile::isAllowableAlgorithmType(E_KeyType keyType) const
{
  OFBool result = OFFalse;
  switch (keyType)
  {
...
OFBool SiCreatorProfile::attributeForbidden(const DcmTagKey& /* key */) const
{
  // we have no special restrictions for this profile
  return OFFalse;
}

OFBool SiCreatorProfile::isAllowableTransferSyntax(E_TransferSyntax xfer) const
{
...
const int sicreapr_cc_dummy_to_keep_linker_from_moaning = 0;
