...
{
	return DiMonoImage::getOutputData(frame, bits, 0, maxval(bits)); 
}


DiImage *DiMono2Image::scale(const unsigned long width, const unsigned long height, const int interpolate) const
{
	DiImage *image = new DiMono2Image(this, (const Uint16)width, (const Uint16)height, interpolate);
	return image;
}
...
{
	DiImage *image = new DiMono2Image(this, (const Uint16)left, (const Uint16)top, (const Uint16)width,
		(const Uint16)height);
	return image;
}
...
{
	DiImage *image = new DiMono2Image(this, (const Uint16)Columns, (const Uint16)Rows);
	return image;
}
