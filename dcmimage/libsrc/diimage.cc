	...
	InputData(NULL)
{
	if ((Document != NULL) && (Status == EIS_Normal))
    {
		...
		if (!ok || ((Rows > 0) && (Columns > 0)))
		{
			...
			checkPixelExtension();
			/*const*/ DcmOtherByteOtherWord *pixel = (DcmOtherByteOtherWord *)Document->search(DCM_PixelData);
			if (ok && (pixel != NULL))
			{
				Status = EIS_Normal;
...
void DiImage::convertPixelData(/*const*/ DcmOtherByteOtherWord *pixel)
{
    if ((pixel->ident() == EVR_OW) || ((pixel->ident() == EVR_OB) && (BitsAllocated <= 8)))
	{
		...
		else if ((pixel->ident() == EVR_OB) && (BitsAllocated <= 8))
		{
