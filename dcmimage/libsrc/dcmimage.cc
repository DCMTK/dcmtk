        ...
        else if (Document->getValue(DCM_PhotometricInterpretation, str))
        {
            ...
            switch (PhotometricInterpretation)
            {
                ...
                case EPI_CMYK:
                    ...
                    break;
                case EPI_YBR_Full:
                    Image = new DiYBRImage(Document, Status);
                    break;
                case EPI_YBR_Full_422:
                    Image = new DiYBR422Image(Document, Status);
                    break;
                case EPI_YBR_Partial_422:
                    Image = new DiYBRPart422Image(Document, Status);
                    break;
    ...
    const int aspect) const
{
    ...
    if ((Image != NULL) && (gw > 0) && (gh > 0)) 
    {
        ...
        {
            if (width == 0)
                width = (unsigned long)(getWidthHeightRatio() * (double)(height * gw) / gh);
            else if (height == 0)
                height = (unsigned long)(getHeightWidthRatio() * (double)(width * gh) / gw);
        }
        else                                                    // ignore pixel aspect ratio
        {
            if (width == 0)
                width = (unsigned long)((double)(height * gw) / gh);
            else if (height == 0)
                height = (unsigned long)((double)(width * gh) / gw);
        }
